...
function getStyles(): string {
  return `
    ...
    * {
    ...
    .critical-box {
      background: ${S.dangerBg};
      border-left: 3px solid ${S.dangerBorder};
      padding: 10px 12px;
      margin: 10px 0;
      border-radius: 4px;
    }
    ...
    .med-profile-header {
      border-bottom: 1px solid ${S.gray300};
      padding-bottom: 6px;
      margin-bottom: 8px;
    }
...
function renderLegalDisclaimer(): string {
  return `
    <div style="
      background: ${S.gray100}; 
      border-left: 4px solid ${S.gray400}; 
      padding: 12px 16px; 
      ...
      color: ${S.gray700};
    ">
      <strong style="color: ${S.gray800};">⚠️ ÄRZTLICHE VERANTWORTUNG:</strong><br>
    ...
    <div style="
      background: ${S.infoBg}; 
      border-left: 4px solid ${S.infoBorder}; 
      padding: 12px 16px; 
      margin: 12px 0 16px 0;
      font-size: 9pt;
      line-height: 1.4;
      color: ${S.infoText};
    ">
      <strong style="color: ${S.infoText};">💡 MEDLESS IST EIN OBERGRENZEN-TOOL:</strong><br>
...
function renderLevel1Overview(data: DoctorReportDataV3): string {
  return `
    ...
    <div class="warning-box" style="background-color: ${S.warningBg}; border-left: 4px solid ${S.warningBorder}; padding: 12px; margin: 16px 0;">
      <strong style="color: ${S.warningText}; font-size: 11pt;">🩺 Organfunktion – Ärztliche Hinweise</strong>
      
      <div style="margin: 12px 0;">
        <p style="margin: 0 0 8px 0; font-size: 9pt; color: ${S.warningText}; font-style: italic;">
      ...
      <table style="width: 100%; border-collapse: collapse; margin: 8px 0;">
        <thead>
          <tr style="background-color: ${S.warningBg}; border-bottom: 2px solid ${S.warningBorder};">
            <th style="padding: 6px; text-align: left; font-size: 9pt; color: ${S.warningText};">Organ</th>
            <th style="padding: 6px; text-align: left; font-size: 9pt; color: ${S.warningText};">Status</th>
            <th style="padding: 6px; text-align: left; font-size: 9pt; color: ${S.warningText};">Klinische Empfehlung</th>
          </tr>
        </thead>
        <tbody>
          ${data.liverFunction && data.liverFunction !== 'normal' ? `
          <tr style="border-bottom: 1px solid ${S.warningBg};">
            <td style="padding: 6px; font-size: 9pt; color: ${S.warningText};"><strong>Leber</strong></td>
            <td style="padding: 6px; font-size: 9pt; color: ${S.warningText};">
          ...
          <tr style="border-bottom: 1px solid ${S.warningBg};">
            <td style="padding: 6px; font-size: 9pt; color: ${S.warningText};"><strong>Niere</strong></td>
            <td style="padding: 6px; font-size: 9pt; color: ${S.warningText};">
      ...
      <div style="margin: 12px 0 0 0; padding: 8px; background-color: ${S.warningBg}; border-radius: 4px;">
        <strong style="color: ${S.warningText}; font-size: 9pt;">Monitoring-Checkliste bei eingeschränkter Organfunktion:</strong>
        <ul style="margin: 6px 0 0 20px; color: ${S.warningText}; font-size: 9pt; line-height: 1.4;">
      ...
      <p style="margin: 12px 0 0 0; font-size: 8pt; color: ${S.warningText}; font-style: italic;">
...
function renderCBDAndReductionSummary(data: DoctorReportDataV3): string {
  ...
  return `
    ...
    ${showCBD ? `
    <div style="background: ${S.successBg}; border-left: 4px solid ${S.successBorder}; padding: 12px 16px; margin: 12px 0; border-radius: 4px;">
      <h3 style="color: ${S.successText}; margin-bottom: 8px;">CBD-Dosis (Start → Ende)</h3>
    ...
    ${reduction ? `
      <div style="background: ${S.infoBg}; border-left: 4px solid ${S.infoBorder}; padding: 12px 16px; margin: 12px 0; border-radius: 4px;">
        <h3 style="color: ${S.infoText}; margin-bottom: 8px;">Reduktionsziel</h3>
        <p style="font-size: 10pt; margin: 4px 0;">
        ...
        <p style="font-size: 9pt; color: ${S.gray500}; margin-top: 6px; font-style: italic;">
        ...
        <h4 style="font-size: 10pt; margin-top: 10px; color: ${S.gray700};">Medikamentenspezifische Reduktionen:</h4>
        <ul style="margin: 6px 0 0 20px; font-size: 9pt; line-height: 1.6;">
          ${reduction.medications.map(med => {
            ...
            if (med.startMg === undefined || med.startMg === null || med.startMg === 0) {
              return `<li><strong>${med.name}:</strong> <span style="color: ${S.gray400};">Keine Reduktion berechnet (Safety-Layer aktiv). Grund: konservatives Vorgehen empfohlen.</span></li>`;
            }
            
            // ✅ D2 FIX: Explain 0% reduction clearly
            if (med.reductionPercent === 0) {
              return `<li><strong>${med.name}:</strong> ${formatMgValue(med.startMg)} → ${formatMgValue(med.endMg)} <span style="color: ${S.gray400};">(0% - Safety-Layer blockiert Reduktion aufgrund pharmakologischer Risikofaktoren)</span></li>`;
            }
            return `<li><strong>${med.name}:</strong> ${formatMgValue(med.startMg)} → ${formatMgValue(med.endMg)} (${med.reductionPercent}%)</li>`;
          }).join('')}
        </ul>
      </div>
    ` : ''}
    
    <div style="background: ${S.warningBg}; border-left: 4px solid ${S.warningBorder}; padding: 12px 16px; margin: 12px 0; border-radius: 4px;">
      <h3 style="color: ${S.warningText}; margin-bottom: 8px;">MEDLESS-Empfehlung</h3>
  ...
  return `
    ...
    <table>
      ...
      <tbody>
        ${medications.map(med => `
          <tr>
            <td><strong>${med.name}</strong><br><span style="font-size: 8pt; color: ${S.gray500};">${med.genericName || '—'}</span></td>
            <td>${med.category || '—'}</td>
...
function renderTaperTailWarning(): string {
  return `
    <div class="warning-box" style="margin-top: 16px;">
      <p style="font-weight: 700; color: ${S.warningText}; margin-bottom: 6px;">⚠️ TAPER-TAIL-WARNUNG (Letzte 25–30% der Reduktion):</p>
...
function renderTwoPercentFloorWarning(): string {
  return `
    <div class="warning-box" style="margin-top: 8px;">
      <p style="font-weight: 700; color: ${S.warningText}; margin-bottom: 4px;">⚠️ SICHERHEITSHINWEIS – 2%-UNTERGRENZE ANGEWENDET:</p>
...
function renderHighRiskSubstanceClassesWarning(): string {
  return `
    <div class="critical-box" style="margin-top: 16px;">
      <p style="font-weight: 700; color: ${S.dangerText}; margin-bottom: 8px;">⚠️ BESONDERS VORSICHTIG ANWENDEN BEI:</p>
...
function renderPharmacokineticsVsPharmacodynamicsNote(): string {
  return `
    <div class="info-box" style="margin-top: 16px;">
      <p style="font-weight: 700; color: ${S.infoText}; margin-bottom: 6px;">🔬 WICHTIGER HINWEIS: PHARMAKOKINETIK VS. PHARMAKODYNAMIK</p>
      <p style="font-size: 9pt; line-height: 1.5;">
...
function renderMonitoringRecommendations(data: DoctorReportDataV3): string {
  return `
    <div class="info-box" style="margin-top: 16px;">
      <p style="font-weight: 700; color: ${S.infoText}; margin-bottom: 8px;">🩺 MONITORING-EMPFEHLUNGEN:</p>
      <ul style="margin-left: 18px; font-size: 9pt; line-height: 1.6;">
...
function renderLevel2MedicationProfiles(data: DoctorReportDataV3): string {
  ...
  return `
    ...
    <p style="font-size: 9pt; color: ${S.gray500}; margin-bottom: 12px;">
...
function renderMedicationProfile(med: DoctorReportDataV3['medicationDetails'][0], index: number): string {
  return `
    <div class="med-profile">
      ...
      <hr style="margin: 12px 0; border: none; border-top: 2px solid ${S.successBorder};">
      ...
      <hr style="margin: 8px 0; border: none; border-top: 1px solid ${S.gray200};">
      
      ${index === 0 ? '<h3 style="font-size: 10pt; margin-top: 8px; color: ${S.successText}; margin-bottom: 10px;">🔬 Modell-Faktoren (gelten für alle Profile):</h3>' : ''}
      ...
      ${med.monitoring ? `
        <hr style="margin: 8px 0; border: none; border-top: 1px solid ${S.gray200};">
        <h3 style="font-size: 10pt; margin-top: 8px; color: ${S.successText};">🩺 Überwachungs-Empfehlungen</h3>
        <div style="font-size: 9pt; color: ${S.gray600}; line-height: 1.5;">
          ${med.monitoring}
        </div>
      ` : ''}
    </div>
...
function renderBasiswerteSection(med: DoctorReportDataV3['medicationDetails'][0]): string {
  return `
    <h3 style="font-size: 10pt; margin-top: 8px; color: ${S.successText}; margin-bottom: 6px;">📋 A. Basiswerte</h3>
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin-bottom: 10px;">
      <div style="background: ${S.gray100}; padding: 6px 8px; border-radius: 4px;">
        <div style="font-size: 8pt; color: ${S.gray500}; font-weight: 600;">KATEGORIE</div>
        <div style="font-size: 9pt; color: ${S.gray800};">${med.category || 'Unbekannt'} ${med.rawData?.categoryId ? `(ID ${med.rawData.categoryId})` : ''}</div>
      </div>
      <div style="background: ${S.gray100}; padding: 6px 8px; border-radius: 4px;">
        <div style="font-size: 8pt; color: ${S.gray500}; font-weight: 600;">HALBWERTSZEIT</div>
        <div style="font-size: 9pt; color: ${S.gray800};">${med.rawData?.halfLifeHours ? `${med.rawData.halfLifeHours}h` : 'Keine Daten'}</div>
      </div>
      <div style="background: ${S.gray100}; padding: 6px 8px; border-radius: 4px;">
        <div style="font-size: 8pt; color: ${S.gray500}; font-weight: 600;">WITHDRAWAL SCORE</div>
        <div style="font-size: 9pt; color: ${S.gray800};">${med.rawData?.withdrawalScore !== null && med.rawData?.withdrawalScore !== undefined ? `${med.rawData.withdrawalScore}/10` : 'Keine Daten'}</div>
      </div>
    </div>
  `;
}
...
function renderCypEnzymeTable(med: DoctorReportDataV3['medicationDetails'][0]): string {
  ...
  if (enzymes.length === 0) {
    return `
      <h3 style="font-size: 10pt; margin-top: 8px; color: ${S.successText}; margin-bottom: 6px;">🧬 B. CYP-Profil</h3>
      <p style="font-size: 9pt; color: ${S.gray500}; margin-bottom: 10px;">Keine CYP-Daten vorhanden (Nicht-CYP-Metabolismus: z.B. UGT, Renale Clearance)</p>
    `;
  }
  ...
  return `
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 9pt; margin-bottom: 10px;">
      <thead style="background: ${S.gray100};">
        <tr>
          <th style="border: 1px solid ${S.gray200}; padding: 6px; text-align: left; font-weight: 600;">Enzym</th>
          <th style="border: 1px solid ${S.gray200}; padding: 6px; text-align: center; font-weight: 600;">Substrat</th>
          <th style="border: 1px solid ${S.gray200}; padding: 6px; text-align: center; font-weight: 600;">Inhibitor</th>
          <th style="border: 1px solid ${S.gray200}; padding: 6px; text-align: center; font-weight: 600;">Induktor</th>
        </tr>
      </thead>
      <tbody>
        ${enzymes.map(e => `
          <tr>
            <td style="border: 1px solid ${S.gray200}; padding: 6px; font-weight: 600;">${e.name}</td>
            <td style="border: 1px solid ${S.gray200}; padding: 6px; text-align: center;">${e.data.substrate ? '✓' : '—'}</td>
            <td style="border: 1px solid ${S.gray200}; padding: 6px; text-align: center; color: ${e.data.inhibitor ? '${S.dangerBorder}' : '${S.gray500}'}; font-weight: ${e.data.inhibitor ? '600' : '400'};">${e.data.inhibitor ? '✓' : '—'}</td>
            <td style="border: 1px solid ${S.gray200}; padding: 6px; text-align: center; color: ${e.data.inducer ? '${S.successText}' : '${S.gray500}'}; font-weight: ${e.data.inducer ? '600' : '400'};">${e.data.inducer ? '✓' : '—'}</td>
          </tr>
        `).join('')}
      </tbody>
    </table>
...
function renderCalculationFactorsBox(med: DoctorReportDataV3['medicationDetails'][0]): string {
  ...
  return `
    <h3 style="font-size: 10pt; margin-top: 8px; color: ${S.successText}; margin-bottom: 6px;">🧮 C. Berechnungsformel (MEDLESS-Modell)</h3>
    <div style="background: ${S.successBg}; border: 2px solid ${S.successBorder}; border-radius: 6px; padding: 10px; margin-bottom: 10px;">
      <div style="font-size: 9pt; line-height: 1.7; color: ${S.gray800};">
        ...
        <hr style="border: none; border-top: 1px solid ${S.successBorder}; margin: 8px 0;">
        
        <div style="font-size: 10pt; font-weight: 700; color: ${S.successText};">
          <strong>FINAL FACTOR:</strong> ${(cf.finalFactor * 100).toFixed(1)}% 
          <span style="font-weight: 400; color: ${S.gray500};">(Produkt aller Faktoren)</span>
        </div>
      </div>
    </div>
    
    <div style="background: ${S.infoBg}; border-left: 4px solid ${S.infoBorder}; padding: 8px 10px; margin-bottom: 10px;">
      <div style="font-size: 9pt; color: ${S.gray800}; font-weight: 600;">
        📌 D. MEDLESS Empfehlung: Max. <span style="color: ${S.successText}; font-size: 11pt;">${med.maxWeeklyReductionPct}% pro Woche</span>
      </div>
      <div style="font-size: 8pt; color: ${S.gray500}; margin-top: 4px;">
...
function renderWithdrawalRiskSection(wr: DoctorReportDataV3['medicationDetails'][0]['withdrawalRisk']): string {
  if (!wr || wr.score === 0) {
    return '<p style="font-size: 9pt; color: ${S.gray500};">Kein erhöhtes Absetzrisiko (Entzugssymptomatik) dokumentiert.</p>';
  }
...
function renderCypSection(cyp: DoctorReportDataV3['medicationDetails'][0]['cypData']): string {
  if (!cyp || !cyp.hasCypData) {
    return '<p style="font-size: 9pt; color: ${S.gray500};">Keine CYP450-Daten vorhanden.</p>';
  }
  ...
  return `
    <div class="med-profile-row">
      ...
      <div class="med-profile-value">
        ...
        <span style="color: ${S.gray500};">(${cyp.effectType === 'slower' ? 'Verlangsamung' : 'Beschleunigung'})</span>
      </div>
    </div>
    ${cyp.clinicalConsequence ? `
    <div class="med-profile-row">
      <div class="med-profile-label">Klinische Konsequenz:</div>
      <div class="med-profile-value" style="color: ${S.dangerBorder}; font-weight: 500;">
...
function renderTherapeuticRangeSection(tr: DoctorReportDataV3['medicationDetails'][0]['therapeuticRange']): string {
  if (!tr || !tr.hasRange) {
    return '<p style="font-size: 9pt; color: ${S.gray500};">Kein therapeutisches Fenster definiert.</p>';
  }
  ...
  return `
    <div class="med-profile-row">
      ...
      <div class="med-profile-value">
        ...
        ${tr.isNarrow ? ' <span style="color: ${S.dangerBorder}; font-weight: 600;">⚠️ ENGES FENSTER</span>' : ''}
      </div>
    </div>
...
function renderMdiImpactSection(mdi: DoctorReportDataV3['medicationDetails'][0]['mdiImpact']): string {
  if (!mdi || !mdi.contributesToMdi) {
    return '<p style="font-size: 9pt; color: ${S.gray500};">Keine Multi-Drug-Interaktion.</p>';
  }
...
function renderFullSafetyNotes(notes: DoctorReportDataV3['fullSafetyNotes']): string {
  ...
  return `
    ...
    ${deduplicatedNotes.map(medNotes => {
      ...
      return `
        <div style="margin: 16px 0; padding: 12px; background: ${S.warningBg}; border-left: 4px solid ${S.warningBorder}; border-radius: 4px; page-break-inside: avoid;">
          <h3 style="font-size: 11pt; font-weight: 700; color: ${S.warningText}; margin-bottom: 8px;">${medNotes.medicationName}:</h3>
  ...
  return `
    ...
    <div class="info-box">
      ...
      ${modelInfo.factorsNotIncluded && modelInfo.factorsNotIncluded.length > 0 ? `
        ...
        <ul>
          ${modelInfo.factorsNotIncluded.map(f => `<li style="font-size: 9pt; color: ${S.dangerBorder};">${f}</li>`).join('')}
        </ul>
      ` : ''}
