-- ============================================================
-- MEDLESS: Default Safety Rules for Medication Categories
-- ============================================================
-- This script populates medication_categories with conservative
-- safety defaults for common medication classes.
-- 
-- IMPORTANT: This is a STARTING POINT, not definitive medical guidance.
-- All values are intentionally conservative and should be refined
-- with expert medical input over time.
-- ============================================================

-- Ensure we have all required categories first (INSERT OR IGNORE)

-- 1) Transplantations-Immunsuppressiva
...
-- 2) Antikoagulanzien / Plättchenhemmer (Langzeittherapie)
INSERT OR IGNORE INTO medication_categories (name, risk_level, can_reduce_to_zero, default_min_target_fraction, max_weekly_reduction_pct, requires_specialist, notes)
VALUES (
...
-- 6) Opioide (chronische Schmerztherapie)
INSERT OR IGNORE INTO medication_categories (name, risk_level, can_reduce_to_zero, default_min_target_fraction, max_weekly_reduction_pct, requires_specialist, notes)
VALUES (
  'Opioide',
  'high',
...
-- 8) Standard / gut reduzierbare Medikamente
INSERT OR IGNORE INTO medication_categories (name, risk_level, can_reduce_to_zero, default_min_target_fraction, max_weekly_reduction_pct, requires_specialist, notes)
VALUES (
  'Standard',
  'moderate',
...
-- ============================================================
-- NOW UPDATE EXISTING CATEGORIES WITH SAFETY RULES
-- ============================================================

-- Update Benzodiazepine category (if exists without full name)
...
-- Update Antidepressivum category
UPDATE medication_categories
SET 
  risk_level = 'high',
  can_reduce_to_zero = 1,
...
-- Update Schmerzmittel to Standard-like values (if low-risk NSAIDs)
UPDATE medication_categories
SET 
  risk_level = 'moderate',
  can_reduce_to_zero = 1,
...
-- ============================================================
-- MEDICATION MAPPING: Assign medications to categories
-- ============================================================
-- This is a heuristic based on generic_name patterns.
-- Medications without explicit category assignment will remain
-- in their current category or get NULL (handled as "Standard" in code)
-- ============================================================
...
-- ============================================================
-- SELECT name, risk_level, can_reduce_to_zero, max_weekly_reduction_pct FROM medication_categories;
-- SELECT m.name, m.generic_name, mc.name as category FROM medications m LEFT JOIN medication_categories mc ON m.category_id = mc.id WHERE mc.name IS NOT NULL;
